...
The build already stamps these headers into `dist/pine-bot.user.js` from
...
1. **Install FROM the raw URL once** (not by pasting the file into the editor):
   open <https://raw.githubusercontent.com/changhakim90/pine-bot/main/dist/pine-bot.user.js>
   in the browser that has Violentmonkey — it offers to install. If you had
   an older hand-pasted copy, remove it first so only one copy runs.
2. **Push every release to `main` with `dist/` rebuilt and committed** (CI
   fails the push if `dist/` is stale). Violentmonkey compares the remote
   ...
   1 hour (default is 1 day), or force it from the dashboard's ⟳ button.

Verify what the update URL is actually serving (raw.githubusercontent can lag
a push by a few minutes):
