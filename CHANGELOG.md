...
| 6.88.3+crown | — | | | | | | | **Roster and scorer rebuilt from a LIVE 373-MINUTE PAT RUN** (`gt 22402`, **121 minutes past the 252:30 crown**, 441/441 HP), read straight off the player object. That run carries `gintonic:6 mojito:6 moscowmule:6 negroni:6 southside:6 vodkatonic:6 whiskysour:6` with `superLv {vodkatonic:6, gintonic:6, mojito:6, southside:6}` — **four supers, not five** — plus `coffee:6` (`reviveCharges: 1`), both crafts absorbed (`syrup`, `blackver`), and **no CAMPARI at all**, which is why its NEGRONI sat at Lv6 and never supered. **`maxSuperLines` 5 → 4** (user), the four being SOUTH SIDE→MINT, VODKA TONIC→TONIC, GIN TONIC→TONIC, MOJITO→SUGAR — three keys covering four lines because TONIC is shared. **MOJITO joins the roster**: the config had excluded it ("SUGAR = pure luck stat") while the deepest run in the project's history was carrying SUPER MOJITO. **COFFEE BEANS joins** and leaves `AVOID_INGREDIENTS_BASE` — it is the revive, and it had been sitting on the avoid list while a comment two hundred lines away valued it at 26. **CAMPARI and COSMOPOLITAN are out** (COSMOPOLITAN's ORANGE key never unbans, so it could never super and never earned its slot). **New scorer doctrine, all from user direction:** `keyless-core` +46/+66 for WHISKY SOUR, NEGRONI and VODKA CRANBERRY — Lv6 cocktails that earn a slot on raw effect with no super path; `super-line` +54/+74 so the four keyed lines are not quietly demoted beneath them (caught on first measurement: SOUTH SIDE had fallen to 97 against NEGRONI's 136); `boss-killer` +28/+50 on SOUTH SIDE ("essential to killing bosses" — it is the only body-centred burn zone in the plan, and the zone predicate hits a boss's hitbox circle rather than needing its centre); `top-ingredient` +38/+52 for OLIVE, TOMATO JUICE, CRANBERRY, MINT and the two crafts; `craft-half` +34 for SWEET VERMOUTH, DRY VERMOUTH, WATER and SUGAR, since a craft is only reachable if both halves hit Lv6; `tonic-two-lines` +32 in the day, +12 after (one ingredient buys half the super plan); `junk-acceptable` +6 for LIME and SODA WATER, which leave the hard-avoid list — worse than anything planned, better than CORPSE REVIVER No.2 or ABSINTHE, which cannot damage holdouts at all. Resulting order at gt 600: SOUTH SIDE 179, NEGRONI 136, VODKA CRANBERRY 109, VODKA TONIC / GIN TONIC / MOJITO 101, WHISKY SOUR 93. **`sp_timestop` promoted, and this is the substantive one.** That run reads `timestopBonus: 162` — +162 SECONDS on every TIME STOP pickup, ~81 stacked `+2s` picks, and **the largest number in the entire player object**, bigger than every damage multiplier, mitigation term and sustain source combined. It matches demo #3 (22 of 31 picks after 26:00 were TIME STOP +2S). The corner and the ult chain keep you alive BETWEEN stops; the stops are what make a 234-enemy field irrelevant. +90 past the deep-deep threshold, +40 in hell before it, old weighting unchanged in the day. **Kept against the evidence, at user direction:** VODKA CRANBERRY over MOSCOW MULE. The 373-minute build carries the mule. **Flagged, not silently resolved:** the user asked for four super lines and then for CRANBERRY as a top-priority ingredient; CRANBERRY is VODKA CRANBERRY's key, so the roster can complete five. Five is still a full line under the six-maxed-super gun gate, so the test asserts ≤5 and the tension is recorded here. |
| 6.88.4+crown | — | | | | | | | **The phase doctrine, encoded as a strict order** (user: "the ordering of everything matters"). **DAY** — the plan is taken in the stated sequence, ingredients before cocktails: OLIVE, DRY VERMOUTH, *SWEET VERMOUTH*, BLACK VERMOUTH, WATER, SUGAR, *SIMPLE SYRUP*, TOMATO JUICE, CRANBERRY, MINT, *TONIC*, SOUTH SIDE, MOJITO, VODKA TONIC, *GIN TONIC*, NEGRONI, WHISKY SOUR, VODKA CRANBERRY. Italics are the three additions I made and flagged in-source rather than assumed: SWEET VERMOUTH and SIMPLE SYRUP because a craft is unreachable without both halves, and TONIC / GIN TONIC because TONIC is the shared key for two of the four super lines and the list named VODKA TONIC without it. **The first implementation of this did not work and the measurement caught it:** a 7-point step per rank is noise against residual terms that span ~150 points (super-line 54, boss-killer 50, progress 46, survival-kit 30), so SOUTH SIDE came out 3rd instead of 12th. For an order to hold, the step between ranks must exceed the entire spread of everything else — it is now 200, and the day ranking is exactly as specified. **Two cards sit above the whole list, and both are the same doctrine rather than an exception to it:** the ULTIMATE ("ultimates used to kill passouts as priority for early loot and reward upgrades" — the day is the funding phase), and SHAKING UP, because super evolution requires *base attack MAX* + cocktail Lv6 + key MAX. Ranking the base below seventeen other cards would have silently deleted the four-line plan the order exists to build. The existing `directives` scenario caught this within a minute of the change. **HELL** — the plan is built; the job becomes not opening the six-maxed-super gun gate. LIME, SODA WATER and COFFEE BEANS become real picks (+26) rather than last resorts. **30–80 MIN** — while bosses still drop tips, a FROZEN boss is a free kill and the tip is the super-evolution trigger, so the stacking station drops from 150 px to the hitbox (`r*0.5`). Gated on the freeze actually holding (>120 frames left): the first cut applied it for the whole window and the existing time-stop tests caught that parking on the hitbox with the pause running out is exactly where the wake-up burst lands. **DEEP HELL** — the corner anchor now fires on the OBSERVABLE signal as well as the clock: a boss ring spanning ≥55% of the canvas triggers it immediately, since "once bosses don't drop tips and the boss damage ring becomes as large as the canvas" is a state, not a time. The 150-minute threshold remains as the fallback for when no boss is on screen. One `time-stop` assertion was rewritten — it encoded the old back-off doctrine this release deliberately reverses — and a matching assertion added that the point-blank station disappears once the window closes. |
| 6.88.5+crown | — | | | | | | | **Both open questions from the 6.88.4 spec closed, at user direction.** **Five super lines, not four** — "vodka cranberry and cranberry is important", and CRANBERRY is VODKA CRANBERRY's key, so the roster completes five: SOUTH SIDE(MINT) VODKA TONIC(TONIC) GIN TONIC(TONIC) MOJITO(SUGAR) VODKA CRANBERRY(CRANBERRY). Five is still a full line under the six-maxed-super Rainbow Gun gate. **MOSCOW MULE out, except as the safe forced-pool pick** — "mule out unless it's the only option that doesn't make a 6th cocktail". Two changes make that true rather than aspirational. First, **GINGER BEER is now banned permanently**: `hellUnbanIngredients` is emptied, because the mule's super key was the only thing it was ever unbanned for, and leaving it unbanned was the sole path by which the mule could complete a sixth super and open the gate. With it banned the mule is safe to eat at any time, by construction. Second, a **last-resort CLAMP rather than a bonus**. A +9 nudge did not work and the measurement said so immediately: the mule still collected generic cocktail credit (`progress+70`, `knockback-to-6+36`) and scored **111 against COFFEE BEANS' 75** — the bot would have actively sought it. A last resort is defined by its ceiling, so the score is clamped to 30. It also had to be a WRAPPER around `scoreCard` rather than a line before the return: the function has several exit points, and clamping at one let later `add()` calls re-inflate the score straight past it. The ceiling is a measured band, not a guess: below every hell-safe junk pick (COFFEE BEANS 76, LIME 53, SODA WATER 49) so the mule is never sought, above true junk (CORPSE REVIVER No.2 20, GINGER BEER 15, ABSINTHE −14) so a junk-only pool eats the mule rather than the one card the roster notes call unable to damage holdouts at all. A first attempt at 14 put it *under* CORPSE REVIVER and the new assertions caught it. Two scenarios rewritten because they encoded the reversed doctrine: `hell-unban` asserted GINGER BEER unbans in hell (it now asserts the opposite, plus that the unban machinery still runs with an empty list — emptying the config must not silently delete the mechanism), and `gun-path`'s cap moved 4 → 5. |
| 6.88.6+crown | — | | | | | | | **6.88.5's first ten runs said the strict ordering was wrong, and they were right.** `supersPerRun` **1.1** against 1.9 (6.87.4, n=65) and 2.1 (6.88.0, n=29); hell rate 0.60 from 0.76; p60 0.10 → 0. n=10 is no verdict, but supers is the fastest-moving column in the table and it moved a long way. **Cause, measured not guessed:** at 200 points per rank the day order became LEXICOGRAPHIC. `OLIVE lv5` scored **3722** against an unowned `SOUTH SIDE` at **1620**, so nine ingredients had to reach Lv6 before a single cocktail was taken — more picks than a day contains. **And it broke the safety layer**: `gun-guard` is −500 and the rainbow ban −1000, which against a +3400 rank bonus are noise. Cards that open a sixth super line stopped being refused, runs drifted toward the Rainbow Gun, and they ended abruptly because NEGRONI's shield was still unbuilt at rank 16. An order must be a strong PREFERENCE that loses to a veto, never a priority that outranks one. Capped at **200 total** (`200 - rank*11`), so every guard in the scorer dominates it again. **SLOT LOCKOUT (user), and this is the real fix.** "The choices become limited towards building a rainbow gun as the game was designed that way ... allowing the bot to fill the cocktail space earlier in the run when choices aren't limited may be one way to permanently lock out rainbow gun." Late in a run the pool narrows until every cocktail on offer walks a gun line — sometimes two choices — so refusing is not available by then. But an occupied slot cannot be filled by a card. Until every plan cocktail is claimed (`lv >= 1`), an unclaimed one gets **+250** and outranks the entire ingredient order; once the slate is full the term switches off and the stated order governs again. The gun line is closed by **occupancy**, not by scoring — which is the only mechanism that still works when the pool is down to two cards. Measured: unclaimed SOUTH SIDE 549 / NEGRONI 455 / lowest-ranked VODKA CRANBERRY all beat OLIVE lv5 322; slate full → OLIVE 352 over SOUTH SIDE lv1 258; `rainbowup` −999 throughout. New `slot-lockout` scenario (44 total) asserts both directions plus the guard-dominance property the 200-per-rank order broke. |
| 6.125.0+crown+joe | — | | | | | | | **Performance + report budget + the immortal stop rule. No play-logic change.** (1) The planner's CPU bill cut ~4x at 40 enemies and ~11x at 260 (node micro-benchmark of the candidate scan): `Math.hypot` → `hyp()` (2.4x, all 109 sites), the 33-candidate scan now iterates pre-partitioned `th.live/field/siegeWalls/ringBosses` instead of re-filtering 260 enemies five times per candidate, and every candidate-invariant value (`charOf()`, `ownedLevels`, the aura-ult `d0e` pass) is hoisted. Root cause of "the script degrades the game": the tick is gated on GAME time, so at medianSpeed 15 the planner ran ~455x per wall-second, and in deep hell each tick cost ~2.9 ms — more than a core. (2) `pineBot.report()` is now budgeted to 60 KB compact JSON (`trimReport`: nulls stripped, non-current `top[]` dropped, empty buckets dropped, then `picks`/`phases` shortened only as far as needed; every step named in `r.trimmed`, per-section KB in `r.sizes`; `reportFull()` is the escape hatch). The 📋 button copies COMPACT JSON — the indent was ~half the paste. (3) `CONFIG.graduation`: the pin advances joe → minguk → pat when a character books 5 consecutive immortal rows (`isImmortalRow`: cap:true, capAt < runCapS, why ≠ saturated, parkT > 0); persisted in `pineBotGraduation`; `r.graduation` + an IMMORTAL summary line; `pineBot.ungraduate(c)`. Tests: `report-budget` (14), `immortal-graduation` (24), all observed failing against 6.124.0 source. |
| 6.126.0+crown+joe | — | | | | | | | **The optimizer ceiling; the count rule. No play-logic change (proven, not claimed).** (1) `node --trace-opt` showed **planMove was never JIT-compiled**: V8 refuses TurboFan above 61,440 bytes of bytecode (Chrome uses the same ceiling) and planMove is ~3,100 lines — the planner has run in the baseline tier for its whole life. Lifting the cap in node alone took a realistic deep-hell swarm (219 in-range enemies pressing on the corner seat) from 2.33 ms to 0.66 ms/tick. The fix is structural: the 640-line candidate loop is now `scoreCandidates(C)`, the danger field `dangerAt()`, the nearest-live pass `nearestIdx()`; the danger field reads pooled Float64Arrays with every per-enemy coefficient precomputed, enemies that cannot contribute to any candidate are culled exactly (cutoff + step), and the nearest-live pass is skipped unless joe's pierce term will read it. With the loop gone planMove itself fits under the ceiling and is optimized too. **Measured (same node harness, real dist): 6.124.0 → 6.126.0 = 1.44 → 0.19 ms/tick uniform, 6.29 → 0.37 ms/tick swarm (17x).** At medianSpeed 15 that is 2.9 cores → 0.17. (2) `latchHellDuringPlay` no longer runs the `innerText` HUD scan (a synchronous style+layout flush) four times a wall-second all day when the lexical `hell` binding exists — the scan is kept only for a page without the binding. (3) **Equivalence proof:** `tools/plan-diff.js` + `test/golden-plans.json` — 4,320 plans (day/hell × joe/pat/minguk × swarm/uniform, digested from 6.124.0) reproduced to 1e-9 by every build since; scenarios `plan-golden-{joe,pat,minguk}` (teeth: `samples` 32→31 fails 229/240). (4) The immortal stop rule is a **COUNT, not a streak** (user): `graduation.count: 5` total immortal builds per character, deaths in between neither reset nor count, persisted per run (`bookImmortal`) so the 240-row audit eviction cannot lose it, backfilled from rows on upgrade, zeroed by `ungraduate`. `r.graduation.counts` / `countNeeded`. Tests: `hell-latch-scan` (7), `immortal-graduation` rewritten (31), all observed failing against the prior semantics. |
| 6.127.0+crown+minguk | — | | | | | | | **Skill is shared across characters; the track record is not (user: "Joe, Pat, and Minguk should have similar roadmaps... same weapon pool, same learnings on cem").** (1) Stale comment cleanup: the `userRoadmap` block in `01-config-data.js` used to describe a Pat-only build (VODKA MARTINI, BLACK VERMOUTH, gatling+flame) that was never actually wired into `charRoadmap.pat` and doesn't exist in the shipped config — leftover from an abandoned direction that misled analysis into treating the (correct, intentional) shared roadmap as a bug. Replaced with the real doctrine: Joe is the training character by design (weakest, so if it works for him it should be easier for Pat/Minguk), and `charRoadmap.joe/.pat/.minguk` are deliberately identical. (2) **The CEM optimizer (movement/threat/strategy), the cocktail/ingredient priority bandits, the contextual bandit, learned enemy-type damage multipliers, spawn-timetable and crown-path bandit are now ONE store shared by all three bartenders** (`loadLearnInner`/`saveLearn`, moved off the per-character `learnKey()` blob and onto `SHARED_KEY`, which already held `versions`/`snapshots`), seeded once from Joe specifically — his store had by far the deepest, most-refined data (Pat hadn't played a single run since 6.85.23, ~40 versions of tuning ago; Minguk had 8). A run under ANY character now improves the search for all three, and none of it goes stale again just because attention moves to a different bartender. `runs`/`history`/`runLog`/`genHistory`/`hof` stay per-character — honest records of who actually played what, which the compare table's `+joe`/`+pat`/`+minguk` rows and the immortal-count rule both depend on. The pre-existing 6.107.0 one-time `enemyTypeMul` ratchet-wipe carries into the migration correctly (a seed that itself predates that wipe is wiped before adoption, not carried over stale). New scenario `shared-skill` (13 assertions: cross-character inheritance on first load, two-way ongoing sharing, per-character fields staying separate, the split being structural not cosmetic, single-migration semantics) — all observed failing against the prior per-character-store code. `store-guard`'s box-reopen sub-test and `cem-heal`'s ratchet-wipe assertion updated for the new storage shape (same guarantees, `{}` instead of a `delete`d/undefined field). |
| 6.128.0+crown+joe | — | | | | | | | **The immortal-count reset, and the bar raised to ten (user: "In the next build I want to rotate among pat, minguk, and joe and have all their immortal build count reset and start from the new version. They should reach 10 immortal build counts").** The joe → minguk → pat rotation itself is unchanged — it already was "rotate among pat, minguk, and joe" (`CONFIG.graduation.order`, since 6.125.0); what changed is the bar and the floor. (1) `graduation.count: 5 → 10`. (2) **Every character's persisted count and graduated flag is wiped, once**, the first time this version's code runs — a migration in the `graduation` store's own load (`01-config-data.js`), guarded by `resetEpoch128` so a reload never re-fires it, and now written to `localStorage` immediately at the point of reset rather than left for the next graduation/`bookImmortal` write to persist incidentally (a report or reload before either of those fires again must see the reset, not the stale pre-reset blob — caught by a teeth check, see below). (3) **The reset also stamps `graduation.immortalEpochVersion`** with the version that performed it, and a new `versionAtOrAfterEpoch()` gate in `immortalRowsCount` (04) ignores any phase-audit row tagged with an EARLIER version. This is the part that makes "start from the new version" true rather than aspirational: the phase audit is a rolling 240-row window, so without the floor a pre-reset immortal row still sitting in that window at the moment of upgrade would silently re-inflate the freshly-zeroed counter back toward its old value on the very first report — the reset would be undone before anyone saw it hold. Rows with no floor to compare against (a graduation store from before this existed) are ungated, same as always. `immortal-graduation` rewritten for the new bar and floor (33 assertions, including two fresh-boot tests against a real pre-6.128.0 store rather than only the `T.setGraduation()` test-surface shortcut) — 9 observed failing against the prior semantics; a further teeth check on the persistence fix itself (the reset silently not surviving until the next incidental write) was caught and fixed the same way, verified failing before the `localStorage.setItem` was added. |
| 6.129.0+crown+joe/+pat | — | | | | | | | **Cluster-aware ult aiming for Pat's spiral and Joe's aura (user: "let's make the bot allow movement very close or inbetween passouts and use ultimates from there... being in the center of many passouts using like a k-means clustering would allow them to kill them more effectively").** Confirmed first: passouts deal zero contact damage on the ground (three separate comments in `05-movement.js` say so directly) — they are pure impassable obstacles, so there was never a reason to stand off from a pile, only a reason to aim badly. The prior aim (`poCx`/`poCy`) was a single distance-weighted mean over every nearby passout. That has a real failure mode: with two piles of comparable size and distance both up, the mean lands in the GAP between them — nobody's actual position — and a fixed-radius spiral/aura fired there catches fewer bodies than standing in either pile alone. Rather than a generic k-means (which needs a k chosen in advance and iterates to converge, without ever knowing the ult's actual radius), this treats every nearby passout as a candidate circle center, scores each by the falloff-weighted (Pat) or flat (Joe) count of the pool within `charOf().ultReach` of it, and aims at the winning candidate's own sub-centroid — the densest REACHABLE cluster, not an average of every pile on the field. Gated on `meleeUlt` (Pat's `spray`, Joe's `aura`) — Minguk's `nuke` hits the whole field regardless of position (existing doctrine, unchanged since 6.86.12: "Harvest is for melee ults"), so he keeps the exact prior whole-pool mean, byte-for-byte. A first pass applied the new scoring unconditionally and regressed `plan-golden-minguk` (3 scenes, 6 sub-tests, dozens of plans) by subjecting his nuke aim to the same clustering — caught immediately by the golden suite, fixed by moving the `meleeUlt` gate above the aim computation and branching on it. New scenario `passout-cluster-aim` (6 assertions: a tight near pile vs. a matching far pile — Pat's and Joe's aim lands on the near pile within a few px of the nearest actual body, Minguk's is pulled clear past it toward the far one) — all 4 clustering assertions observed failing against the prior single-mean code, both Minguk assertions observed passing throughout (his path is untouched). **Equivalence:** `plan-golden-minguk` unchanged (byte-identical, as intended); `plan-golden-joe`/`plan-golden-pat` golden refreshed against this build (`tools/plan-diff.js --golden`) since the aim change is intentional for them — 1,694 of 4,320 total plan-diff plans differ between the pre- and post-change builds, all confined to joe/pat scenes with 2+ passouts in range. |
| 6.129.1+crown+pat | — | | | | | | | **Version bump only — identical code to 6.129.0.** Needed because a push from a stale cloud-session checkout briefly replaced `origin/main` with 6.124.0-era code stamped `6.129.0`; Violentmonkey installed that copy, and since the real 6.129.0 carries the same `@version` it would never auto-update over it. Bumping forces the replacement. The ten runs booked as `6.129.0+crown+joe` are that mis-stamped build (no graduation rule, per-character stores, fresh CEM) and must not be read as evidence about 6.129.0's aiming change. **Side effect to know about:** the 6.124.0-era `saveLearn` rewrites the shared store with only `versions/snapshots/lastVersion`, so those runs wiped the shared CEM/bandit skill (`skillShared6127` flag included); on the real code's next load the 6.127.0 migration re-seeds from Joe's per-character blob, which now holds that build's 10-run state. Each character's hall of fame survived (per-character, untouched), and `refitCem` takes `hof.slice(0,3)` as three of its five elites, so the search is pulled back toward the trained vectors within a couple of generations. |
| 6.130.0+crown+joe/+minguk/+pat | — | | | | | | | **The immortal count reset again, and the character rotates EVERY run (user: "Let's restart the immortal build process again with Joe, Minguk, and Pat by resetting the counter and starting from 0 with the goal to getting to 10. For this version, let's rotate the character per run instead of sticking with one character until it reaches the 10 immortal build count" — and, correcting the first cut of this build, which took a graduate out of the cycle: "no I want them to rotate on every session regardless of whether they graduated").** (1) A second one-time reset in the `graduation` store's own load (`resetEpoch130`, alongside the 6.128.0 guard — a store with neither flag gets both): every count and graduation wiped, `immortalEpochVersion` re-stamped at this version so rows booked under 6.128.x/6.129.x cannot count toward the fresh ten, persisted immediately, and the new round-robin cursor cleared so the cycle starts at joe. (2) `CONFIG.graduation.rotate: true` — `graduationPick` is a plain round-robin over `order`: every run start picks the character after `graduation.lastPlayed`, cyclically, joe → minguk → pat → joe …, and the cursor is persisted so a reload mid-cycle continues rather than restarting. **It never skips anyone.** Reaching ten is still recorded on the next pick (the graduation record, the 🎓 in the summary, `r.graduation.graduated`) because that is the goal being measured — but a graduate keeps playing, and when all three have reached ten the rotation simply carries on. `rotate: false` restores the 6.125.0 pin-until-graduated path unchanged (kept, tested with the flag explicitly off). `preferredBartender` is inert while `rotate` is on. The report's `graduation` gains `rotate` and `lastPlayed`; `playing` is now a preview of the NEXT pick (computed without moving the cursor or recording anything). What this costs and doesn't: the per-character sample rate splits three ways (each compare row moves a third as fast — expect ~3x the runs before a z on any one row means anything), but the LEARNING does not split — since 6.127.0 the CEM/bandit skill is one shared store, so every run improves all three. **No play-logic change**: the golden-plan scenarios pass unchanged. Tests: `immortal-graduation` +9 round-robin/reset assertions (the pin block now runs with `rotate:false` explicitly and is unchanged), `rotation` +1 (the round-robin outranks the pin) — 11 observed failing against the 6.129.1 build, and the three "graduates keep playing" assertions observed failing against the first (drop-out) cut of this build. |
| 6.131.0+crown+joe/+minguk/+pat | — | | | | | | | **The race ledger — who reaches ten immortal builds cheapest (user: "Is there a way to find out who reached immortal build the fastest and with the fewest runs"). Instrument only, no play-logic change.** The counter says WHERE each character is and never what it cost to get there. `compare` came close — its per-version rows carry per-character `runs`, and 6.130.0's reset happens to coincide with a version bump so those rows align with the epoch — but that alignment breaks the moment any version ships mid-race, which here is the norm. So `bookImmortal` now keeps a per-character epoch ledger in the graduation store it already owns (`graduation.progress[char]`): `runs` played since the ledger opened, and for every immortal build a `marks[N]` entry with the run number, the wall clock, and the `capAt` that proved it. Marks are read off the COUNTER rather than off `isImmortalRow`, so the ledger cannot drift from the number it measures (the counter also backfills from the audit and can rise by more than one). `graduationStatus()` gains `race` plus three separately-reported winners, because they can disagree: **`fewestRuns`** (the run the target landed on — efficiency), **`fastestWall`** (hours from the ledger opening), and **`fastestBuild`** (median `capAt` — how early in a run each build proved itself; a low median means immortal sooner, not just eventually). A ledger opened mid-epoch adopts the standing count into `adopted`, which makes that character's `runsTo` an explicit LOWER BOUND rather than a silent understatement — stated in the report note. The summary carries it at a glance: the IMMORTAL line gains ` in N` (runs; `N+` while still counting) and a new RACE line names the three winners with each character's immortal/runs and median cap. Tests: `immortal-graduation` +6 (run-number stamping, deaths cost a run but never a mark, per-character ledgers do not pool under the rotation, the three winners with a case where fewest-runs and earliest-cap disagree, the mid-epoch adoption caveat, both summary lines) — all six observed failing against the 6.130.0 build. |
| 6.132.0+crown+joe/+minguk/+pat | — | | | | | | | **The immortal bar is now the BUILD, not the super count (user: "let's remove this from the rule and instead replace it with has southside, simple syrup, olives, and sweet vermouth or black vermouth all maxed out"), plus the multi-tab ledger fix.** **(1) `capStable.supersMin: 3` is gone**, replaced by `capStable.build` — four clauses, AND across and OR within: `[['SOUTH SIDE'], ['SIMPLE SYRUP'], ['OLIVE'], ['SWEET VERMOUTH','BLACK VERMOUTH']]`. The live report that prompted this had hp and defense both passing and `supers: 2` alone holding the gate shut, with `streakS: 0` and ~1,090 game-seconds left; the 6.103.0 funnel had already recorded supersMin as the binding constraint on nearly every ready build. Supers are also the part of the build the bot steers least — a line arms only when the pool happens to offer its key's last level — while the planner drives all four of these ingredients on purpose. **(2) It reads `player.weapons`, NOT `ownedLevels`** — this is 6.91.2's lesson and the single most expensive naming mistake in the project. A live probe at gt 2218 read `player.defense` 34.992 (the cap) while `ownedLevels['OLIVE']` read **1**, because in-run upgrade levels land under `OLIVE UP` and the bare key freezes at 1 on acquisition; four armour gates spent four versions comparing 1 against thresholds of 2, 4 and 6 and deep park never engaged in a single real run. A gate keyed on `ownedLevels['OLIVE']` would have reproduced that failure exactly. The weapon keys are the ones three live dumps recorded in this repo — lowercase, squashed and **abbreviated**: `southside`, `syrup`, `olive`, `sweetver`, `blackver`. Note `sweetver`, not `sweetvermouth`: a generic squash of the display name does not produce the key the game uses, so the measured spellings are tried first and the squash is only a fallback, with `ownedLevels[name+' UP']`, then the bare key, then `keyEffectivelyMaxed`/`everMaxed` behind it. `applyCraft` leaves consumed materials in `weapons` at full level, so the reading survives the BLACK VERMOUTH craft that eats SWEET VERMOUTH — and the OR clause covers it either way. **(3) The gate self-reports.** `capStatus().build` (and therefore `report().cap.build`) carries every clause with the key that actually answered and where the number came from, plus the raw weapons map; `short.buildShort` names the clauses that are short with their live levels. If a spelling is wrong the first report after install shows `key: null, src: 'none'` instead of the rule quietly never firing again. `capLastResetReason` now also names the short leg when NO streak was running — the old code wrote that field only on a reset, which is why the user's paste showed `streakS: 0` with nothing saying why — and for the build leg it names the clause (`build:SIMPLE SYRUP`). **(4) A third one-time reset** (`resetEpoch132`, alongside the 6.128.0 and 6.130.0 guards): the counts standing in the store were earned against a different bar, so a graduation mixing the two proves nothing. Counts, graduations, the round-robin cursor and the race ledger all go; `immortalEpochVersion` is re-stamped here. **(5) The multi-tab ledger fix.** `bookImmortal` and `graduationPick` now re-read the graduation store from localStorage first, guarded on the current reset flag so a pre-reset blob can never be adopted back. Caught in a live 6.131.0 paste: `compare` showed 4/8/6 runs per character while the race ledger read 1/1/1, and minguk's `runRange` [618,621] covered four run numbers for eight counted runs — two tabs were each writing the whole blob and clobbering the other's progress. The learned stores had had this since 6.113.0; the graduation store never did. **Cost control:** the planner tick is gated on game time, so at medianSpeed 15 the gate would be called ~455x per wall-second; levels only go up, so the pass is monotone and latches for the run, with a 1 s recompute below that and a forced fresh read for the report. The latch is cleared at every run boundary — the first cut cleared it only in `startRun` and six scenarios went red, which is also the production hazard (a stale latch would cap a run immortal before it bought a card). **Equivalence:** `plan-diff` against the shipped 6.131.0 prints `4320 plans compared, 0 differ`; the golden-plan scenarios pass unchanged. The build gate is the intended behaviour change and the golden scenes do not reach it. **Tests:** `run-cap` +11 (the four clauses one level short, BLACK VERMOUTH alone satisfying the vermouth clause, an empty list as the off switch, a maxed weapons map passing with the frozen `ownedLevels` OLIVE:1, the per-run latch, the named reset reason, the config shape and `supersMin` being absent), `immortal-graduation` +5 (three multi-tab assertions, the third reset wiping a 6.131.0 store and its race ledger). Teeth verified individually: removing the gate fails 6, dropping the measured `blackver` spelling fails 1, reading `ownedLevels` first fails 1, failing closed on an empty list fails 1, not clearing the latch at a run boundary fails 6, dropping `resetBuildGate` from the reset fails 2, and reverting `reloadGraduation` fails 2 of the 3 multi-tab tests (the third fails when its reset guard is removed instead). |
...
| 6.132.2+crown+joe/+minguk/+pat | — | | | | | | | **A CRAFT RESULT IS BINARY, and 6.132.0 read it as a level — the build gate refused a build that had everything it asked for (user: "this should be counted as an immortal build", then "simple syrup disappears from the ingredients list like the black vermouth once crafted").** The live report at gt 5179 settles it beyond argument: `player.weapons` held `water 6, sugar 6, syrup 1` and `sweetver 6, dryver 6, blackver 1`, with **every other key at 6**. Both crafts had fired; the two craft RESULTS are the only entries not at 6, and they sit at exactly 1 because a crafted item leaves the ingredient pool — it can never be offered again, so its level can never move. **1 is COMPLETE, not 1-of-6.** 6.132.0 compared that 1 against `ownedMax[name] || 6` and held the gate shut on a run at `def: 35` (the 34.992 cap), `hp: 1.00` and four supers. That is the SAME failure as the `ownedLevels['OLIVE']` trap — a value meaning "owned" read as a level against a threshold of 6 — reproduced inside the very gate written to avoid it, one version later. **The fix:** a name that is an `EVOLUTIONS` *result* has max 1, hard-coded rather than `min(1, ownedMax)`, because `ownedMax` can be seeded from a card the pool offered before the craft and would put the bar back at 6. **What the self-report bought:** `cap.short.buildShort` printed `SIMPLE SYRUP 1/6 (weapons)` — the clause, the live level and the source — so this surfaced in ONE run. The OLIVE trap of the same shape cost four versions and was found only by accident. That is the entire argument for a gate that reports its inputs, and it is now paid for twice over. **A note on the first diagnosis, which was wrong:** at gt 990 the same leg read `key: null, src: 'none'` with no syrup-like key anywhere in the map, and `craft.ready` has read 0 across 1,082 runs — so it was reported as "the bot never crafts syrup". It does; the craft simply had not happened that early, and the craft audit's `ready` counter is separately broken (it never fires even though both crafts demonstrably land — open). One report at one moment in the run is not the run. **Tests:** `run-cap` +3 (`syrup: 1` is complete, `blackver: 1` satisfies the vermouth clause, an UNCRAFTED syrup absent from the map still fails), and the "one level short" loop now UNCRAFTS the binary key instead of docking a level it cannot have. `WEAPONS_FULL` carries `syrup: 1`, so every early-cap scenario now runs against what a real complete build actually looks like. Teeth: reverting to the 6.132.0 max fails 9; passing an absent key fails 3. `plan-diff` against 6.131.0 still prints `4320 plans compared, 0 differ`. **A fourth one-time reset (`resetEpoch1321`) — for the RACE, not the counts.** The counts needed nothing: they were 0/0/0 and honest, because a gate that cannot fire books nothing false. What needed clearing is `progress[char].runs`. Roughly 230 runs (joe 77, minguk 75, pat 78) were played under 6.132.0's gate, and no build could satisfy its SIMPLE SYRUP clause at any point, so those runs never had a chance at an immortal build. Leaving them in the ledger would inflate `runsTo`, `perRun` and `hoursTo` for whoever finishes first — which are the three figures the race exists to report, so the race would have been measuring the broken gate rather than the character. `reloadGraduation`'s adoption guard moves to the new flag in the same commit, so a 6.132.0 blob in another tab cannot be pulled back in after the reset. Tests: `immortal-graduation` +3 (a 6.132.0 store — all three prior guards, a zero count and a full ledger — is wiped and re-stamped; the race then reports 0 runs rather than 77/75/78; a second boot of the already-reset store keeps what it has earned since). Teeth: removing the fourth guard fails 2. |
| 6.133.0+crown+joe/+minguk/+pat | — | | | | | | | **THE SLOT DOCTRINE — occupancy is the only defence that survives a forced pool (user: "the bot needs to fill up the cocktail spaces with junk to not be forced to pick ingredients or upgrades that will lead to a rainbow gun ... it should not upgrade weapons and ingredients upgrades despite the weights making this the correct decision").** **(1) The keyless-occupant premise was never enforced.** A live 6.132.x run at gt 5179 read `whiskysour 6` and `lemon 6` — SUPER WHISKY SOUR armed, displacing VODKA TONIC among the four supers. WHISKY SOUR/NEGRONI/MOSCOW MULE sit in `PLAN_COCKTAILS` as deliberate KEYLESS occupants, safe only while their avoid-listed keys stay under 6. `gunPathProgress` skipped those lines TWICE (as plan cocktails, and via `NEVER_UNBANNED`) and returned 0, so there was no gun-path tax AND — the expensive half — `forcedGunPool` is `every(risk > 0)`, so a pool holding LEMON never counted as forced and **no re-roll was ever spent on it**. `arming-cap` did fire at lv5, but that is a −700 SCORE, and in a two-card hell pool −700 wins; over hundreds of hell level-ups "least bad" compounded into a maxed LEMON. The error is a conflation: "never unbanned" means WE NEVER PROMOTE IT INTO THE PLAN, and it was read as IT CAN NEVER BE MAXED. Now only `SUPER_LINE_COCKTAILS` — the four lines we intend — are exempt, so a keyless occupant whose key is climbing registers as risk, gets taxed, and lets its pool trip the re-roll. A second instance landed in the very next report: `lime: 6`, through the same −700. **(2) The ingredient reserve.** The bar holds EIGHT. Measured at gt 5079: olive/tomato/mint/sweetver/dryver/sugar/tonic/cranberry, all at 6, all eight slots gone, **no WATER and no blackver** — the BLACK VERMOUTH craft was owed (`sweetver 6 + dryver 6`) and had not fired, so the vermouths kept their slots and the bar never reopened. WATER could no longer be ACQUIRED, SIMPLE SYRUP was impossible, and the gate reported it as the only short leg while def 35 and hp 1.00 both passed — **that run was unwinnable from the moment its eighth slot filled**. It is a LOCK, not a bid, and that is the whole design: the first cut gave reserved parts +400 and immediately displaced the card it existed to protect (WATER 345 vs SUGAR 691) and shifted two day-order scenarios. Occupancy is structural instead — while N build parts are unowned the last N slots (capped at 3, since reserving five of eight from the third pick locked out the day build entirely) are simply not on offer to anything else, clamped below every consumable exactly as `slot-burn` clamps a banned pick. `CRAFT_RESERVE` is **derived from `capStable.build`**, not hand-listed: the first cut hardcoded WATER and the next report lost a different part (`dryver 6`, no `sweetver`) with WATER and SIMPLE SYRUP both fine. **(3) Slot-wasters repriced.** OLD FASHIONED (key ANGOSTURA) and CORPSE REVIVER NO.2 (key ABSINTHE) scored −300 as "buys nothing with a slot" — priced before 6.92.0's arming cap existed and never revisited. From `slotUrgency.junkFromS` (900 s), or the moment hell latches, they flip to a +40 `slot-occupant` claim, gated on `safeOccupant()` which requires the key to still have HEADROOM — the LEMON lesson applied, so a waster whose key has crept up is NOT repriced. In the day they stay −300. **(4) The craft audit was blind for 1,079 runs.** `ready` read `ownedLevels[pair] >= 6` — the bare acquisition key, which freezes at 1 while real levels land under `<NAME> UP` (the 6.91.2 trap, THIRD instance in this codebase). So `ready` was structurally 0 and the one question the audit exists to answer — does the prompt never appear, or does it appear and we miss it — was unanswerable by construction. Reads `buildKeyLevel` now, the same game-state source as the build gate. **(5) A fifth one-time reset** (`resetEpoch133`), at the user's request and earned: this changes the PICKER, so builds assembled under the old one are a different experiment. **Equivalence:** `plan-diff` against 6.131.0 prints `4320 plans compared, 0 differ`. **Tests:** `gun-path` +4 (a keyless occupant with a climbing key registers risk, the cocktail too, risk rises with the key, an intended line stays untaxed), `latent-line` +4 (both wasters repriced as occupants in the urgency window, still wasters in the day, the reserve locking the last slot, levelling an owned ingredient untouched, the reserve covering SWEET VERMOUTH, the lock lifting once every part is owned), `craft-prompt` +3 (a craft owed in `player.weapons` is booked, the pair is named, the frozen `ownedLevels` 1 does not hide it), `immortal-graduation` +1 (the fifth reset). Teeth verified individually: restoring the risk exemption fails 3, removing the lock fails 1, removing the occupant repricing fails 2, restoring the blind audit reader fails 3, hand-listing the reserve instead of deriving it fails 1, removing the fifth reset guard fails 2. **Open:** the arming cap has now demonstrably failed twice (LEMON, LIME) — it is a score, and a score cannot survive a two-card pool; the slot work reduces forced pools but does not eliminate them. Also open: `rainbow-rush` at `03-scoring.js:1805/:567` still pays a bonus keyed to SIX supers against a `maxSuperLines` of 4, in hell, which actively pushes toward the gate. |
| 6.133.1+crown+joe/+minguk/+pat | — | | | | | | | **The vermouth clause is DROPPED from the immortal rule (user: "it seems that one run where sweet vermouth or black vermouth wasn't carried still ended with an immortal build run ... if they don't need it then maybe we should drop it from the immortal build requirement").** The gate is now three clauses: SOUTH SIDE, SIMPLE SYRUP, OLIVE. **The stronger reason is one the user did not ask for: the clause was MANUFACTURING the rainbow gun.** `MANHATTAN`'s super key IS `SWEET VERMOUTH` (`SUPER_KEY_INGREDIENT`, 01:1945), and this codebase already names MANHATTAN first among the six cocktails that are "latent by construction" precisely because the plan maxes their keys on purpose. `armsOffPlanLine` returns **false** for SWEET VERMOUTH — it is a `PLAN_INGREDIENT` — so the arming cap is structurally incapable of protecting that line. Measured at gt 2795: `manhattan 6` and `sweetver 6` in `player.weapons`, i.e. **SUPER MANHATTAN armed**, with the pool having offered `★ SUPER MANHATTAN UP` at gt 1884 — a fifth line against a `maxSuperLines` of 4, and the immortal rule itself required the ingredient that armed it. The gate was working against the gun doctrine it shares a config file with. **NO RESET.** The bar is RELAXED, not redefined: every build already counted met a strictly harder requirement and still qualifies, so the five earned this epoch (joe 1, minguk 2, pat 2) stand. Each of the previous four resets followed a change that made old counts incomparable; this one does not, and resetting would have discarded honest data for symmetry. **The reserve follows automatically.** `CRAFT_RESERVE` is derived from `capStable.build`, so SWEET VERMOUTH and DRY VERMOUTH left the reserve with the clause and two ingredient slots were released — the payoff for deriving it in 6.133.0 rather than hand-listing. **What the same report shows working:** the gate fired (`build.ok: true`, all legs green, `readyAt: 1684`), and the binding constraint has moved off the build entirely — `bestStreakS: 156` against `holdS: 300` with `lastResetReason: "hp"`. That is the intended shape: the build completes, and the HP hold is what has to be earned. **Tests:** `run-cap` +2 (three clauses, not four; a build with neither vermouth nor the craft still proves — the assertion that catches the clause being silently reinstated) and +1 in `latent-line` (the reserve drops the vermouths and keeps WATER/OLIVE); the one-level-short loop drops its vermouth case and the reserve tests move to OLIVE. Teeth: reinstating the clause fails 2. `plan-diff` vs 6.131.0 still `0 differ`. **Open, and now urgent:** the MANHATTAN line got to 6 with no `latent-line` tag on the pick at gt 1740 (`progress+70 dps-race+13 boss-dps+8`), so `latent-line-ceiling` is not holding what it was written to hold. Dropping the clause removes the plan's own reason to max SWEET VERMOUTH, but the guard is still not doing its job. |
| 6.134.0+crown+joe/+minguk/+pat | — | | | | | | | **THE ` UP` CARD FIX — the scorer had never once seen a level-up.** The game emits two cards per thing: an acquisition card (`OLIVE`) and a level-up card (`OLIVE UP`). `baseNameOf` stripped `Lv3` and `+1` but **not** ` UP`, and all 39 name-keyed lookups in `03-scoring.js` — `COCKTAILS.includes`, `PLAN_INGREDIENTS.includes`, `SUPER_KEY_INGREDIENT[]`, `INGREDIENT_TAGS[]`, `AVOID_INGREDIENTS.has`, `SLOT_WASTERS.includes` — therefore matched only the acquisition form. Measured in the harness (gt 1740, hell, both forms at lv5): `OLIVE` **262 → `OLIVE UP` 9** (eight plan terms lost), `WATER` 80 → 9, `LIME` **−704 → 9** (arming-cap lost), `LEMON` −679 → 9, `CAMPARI` −684 → 9, `MANHATTAN` −1065 → 71 (latent-line lost), `VODKA CRANBERRY` −983 → 97, `GIMLET`/`OLD FASHIONED` −41 → 71 (gun-path lost). **Every passive level-up scored exactly 9** (`ingredient+8 ucb+1`) whether it was the build's most important ingredient or a banned gun key, so ingredient level-ups were decided by UCB noise alone — and **12 of 14 picks in a live log were ` UP` cards.** **This is the root cause behind findings filed separately for months:** WATER never reaching 6 (so SIMPLE SYRUP never crafts), OLIVE stalling at 5 (`park`: SEATED `medianEntryDef` 35 / 2458 s against NEVER-PARKED 18 / 1373 s — the largest split in the dataset, decided by noise), the 6.118.0 regen spine changing nothing (it added terms to a card taken once), LEMON/LIME/CAMPARI maxing through a −700 arming cap, MANHATTAN and VODKA CRANBERRY arming a fifth line, and `ownedLevels['OLIVE']` frozen at 1 (6.91.2). One bug, six symptoms, ~5,000 runs. **The regex is `/\s+UP$/` and not `/UP$/`:** SIMPLE SYRUP ends in "UP" with no space before it, and a bare anchor would rewrite it to "SIMPLE SYR" and silently unhook the whole regen half. ULTIMATE cards are keyed on `type === 'ult'`, never by name, so they are unaffected. **Why it survived so long:** all ~1,900 existing assertions score by base name (`sc('OLIVE','passive',3)`), because that is how a person writes a test — every guard was verified against a card shape the game rarely offers. The new `up-cards` scenario is that missing axis. **Also, the reserve now follows the build the user actually named** ("simple syrup, mint, and olives are vital ingredients along with supersouthside level 6 as the main weapon for boss killer"): a COCKTAIL clause reserves its SUPER KEY, so SOUTH SIDE reserves MINT, and a CRAFT RESULT reserves itself as well as its parts. A live 6.133.0 run held `southside: 6` with **no `mint` at all** — SUPER SOUTH SIDE impossible — while `bloodymary 6 + tomato 6` and `vodkamartini 6 + dryver 6` had both armed. **The craft audit fix from 6.133.0 reported back immediately:** `ready` went from a structural 0 across 1,084 runs to **859** (`SUGAR+WATER` 108, `SWEET VERMOUTH+DRY VERMOUTH` 751) with `seen: 0` — so the prompt genuinely never appears; SIMPLE SYRUP still crafts (the game auto-crafts it) while BLACK VERMOUTH sat owed for 751 ticks and never fired. That is now a measured fact rather than a hypothesis. **Sixth one-time reset** (`resetEpoch134`): this changes what the bot picks on ~12 of every 14 picks, so earlier builds were assembled by a different bot. **Equivalence:** `plan-diff` vs 6.131.0 still prints `4320 plans compared, 0 differ` — the change is to card scoring, not `planMove`, and the golden scenarios pass untouched. **Tests:** new `up-cards` scenario, 14 assertions (arming-cap on LIME/LEMON/CAMPARI UP, latent-line on MANHATTAN/VODKA CRANBERRY UP, gun-path on GIMLET/OLD FASHIONED UP, OLIVE UP and WATER UP carrying their plan terms, plan and junk level-ups no longer scoring the same, SIMPLE SYRUP surviving the strip in both forms, and the reserve holding MINT/SIMPLE SYRUP/OLIVE). Teeth: removing the ` UP` strip fails **12**; dropping the super-key reserve fails 1. Two pre-existing tests were revised on purpose — the pick log now records `STIRRING` rather than `STIRRING UP`, and "every build part owned" now includes MINT and SIMPLE SYRUP. **Expect the balance to move.** Junk level-ups scored 9 and now carry penalties; plan level-ups scored 9 and now rise sharply. The CEM means in `learning.params` were all learned under the broken scorer and should be treated as suspect until they re-converge. |
| 6.135.0+crown+joe/+minguk/+pat | — | | | | | | | **PLAYER CONTROLS + THE AUDIT (user: "I want to be able to start and stop the bot and end the loop cycle for training. The bot should be able to allow the player to select the character then start the bot" / "diagnose, fix bugs, fix contradictions, fix dead code").** Panel row 3: character chips `⟳ auto / joe / minguk / pat` — a pin bypasses the rotation at the NEXT run start, the cursor is untouched so un-pinning resumes the cycle; `chooseBartender()` now reads the pin FIRST and the legacy chain only when there is none (the dead `bartenderRotation` branch had been overwriting the pin on the second call). Panel row 4: `🔁 Loop runs` / `① One run, then stop` — armed at `startRun()`, consumed at the end of `finishRun()` so the run is booked as always and THEN the bot stops on the results screen; `over()` / `highscore()` / the cap-stage-3 path all `return` when `!running` so nothing presses RETRY. Both persist in `pineBotControl` (own key, in `STORE_BASES`, untouched by any counter reset). Console: `pineBot.pin()`, `.loop()`, `.control()`. Audit (every finding confirmed by execution before it was fixed): `rainbow-rush` was a tautology (`supersThisRun < 4` on a card that IS a super) — now `super-rush`, gated on the card's key being > 0; MOJITO was on `AVOID_COCKTAILS` while also being a plan super line — removed; `LAST_RESORT` / `LAST_RESORT_CEILING` were dead — removed, `scoreCard` is a pass-through; `applyHellUnban` logged an empty list every run; `buildKeyLevel` carried an impossible ` UP` fallback; `pineBotRotIdx` was the one raw (un-namespaced) key; `pineBotGraduation` was missing from the store migration list; `bookImmortal` swallowed its write error (now logs + sets a `⚠ could not save` status once); `GRADUATION_EPOCH` is one constant shared by the store init and `reloadGraduation`; the runner reports a 30 s scenario TIMEOUT distinctly from a failure; five tests that asserted falsified invariants (E1–E4, E7) now assert what the code does. Kept deliberately: the '6.79' profile's dead `!CROWN` branches (A/B alternative). Docs: `claude/current-state.md` (new, start here), `rainbow-lockout.md` / `immortal-stop-rule.md` / `release-state.md` corrected in place with the history kept, README rewritten as the player manual, `CLAUDE.md` doc table updated. New scenarios `up-cards` (+A1/A4) and `controls` (18). `plan-diff`: 4320 plans compared, 0 differ. **No counter reset.** |
| 6.136.0+crown+joe/+minguk/+pat | — | | | | | | | **THE SHIPPED SKILL (user: "make it so that the learnings are built in on the script itself so when a new player starts it doesn't have to play multiple runs").** A fresh install used to start every learner from zero — the CEM at the CONFIG defaults with sigma at 25% of every box, the enemy-type fear table empty (nothing applied until 8 sole hits per type), the boss timetable on the source-extracted fallback — and paid the 20–30 runs per dimension the learning doc quotes, in full, before it played like the trained bot. `SHIPPED_SKILL` (part 01) now carries the reference store's learned state, copied verbatim from the 2026-09-03 📋 report (`6.123.0+crown+joe`, 9,569 runs, CEM gen 754): all 29 CEM means with their converged sigmas, the six enemy fear multipliers with their sole-hit counts (drunk 1.385 / runner 1.356 / boss 1.385 / genz 1.098 / thrower 1.012 / bomber 1.0), and the boss census medians (walls 155 s, first tier 270, then 315 and 360 — not the 120/240/480 fallback). **Seeding is gated on emptiness, never on a flag:** a store with no CEM of any kind starts on the shipped means (`loadLearnInner`, replacing the `DEFAULT_PARAMS` seed), an empty fear table and an empty timetable are filled; a store that has any of these keeps its own untouched — the 9,569-run store sees no change at all. Sigma is re-floored to `learning.sigmaSeed` = 10% of each box (between the 5% floor and a 25% cold start) because the reference predates the 6.134.0 ` UP` fix and is expected to move; a dim whose box no longer matches the box it was trained under falls back to the CONFIG default with a full sigmaInit, like a new dimension. `learning.shippedSkill: false` is the cold-start switch. **Not shipped:** the per-card item/LinUCB tables and the tag bandit — the report carries only summaries, and in the reference store both context layers sit at their caps (+12/+8) for every card with data, so they add a constant, not a ranking; the doctrine scores carry the picks. Report: `learning.seeded` (provenance, dims, sigmaSeed) / `enemySeeded` / `spawnSeeded`; `report().learning.cem.seeded`. **Tests:** new `shipped-skill` scenario (23): every shipped key is a live dimension, inside its box, with its trained box equal to the live box (a widened box would ship a stale corner), every live dim shipped, fear inside the applied band; a fresh store boots on it and APPLIES it at boot, converged sigmas re-floored, wider ones kept, provenance recorded at gen 0; an experienced store keeps its mean/generation/fear/timetable and backfills a missing dim at the CONFIG default; the off switch is a true cold start. Four scenarios that read the "fresh store" as "CONFIG defaults" were re-anchored (`runaway-guard`, `nudge-ratchet`, `lane-escape`, `cem-heal` — the last now asserts the ratchet is gone rather than the table empty), and `plan-scenes.js` clears the fear table beside `applyDefaults()` so the 6.124.0 golden set is still a test of the planner (seeded fear moves 237/240 plans, which is the feature, not a regression). `golden-plans.json` untouched. **No counter reset.** |
